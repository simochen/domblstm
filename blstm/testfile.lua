...
for i = opt.start, opt.ed do
	mfile = string.format("hs%d_lr%.0e_w%d_tg123_ep%d.t7",opt.hiddenSize, opt.lr, opt.weight, i);
	print(mfile)
	model = torch.load(mfile)
	
	terr = predict(model, criterion, trainset)
	print(terr)
	cverr = predict(model, criterion, cvset)
	print(cverr)
	...
	if #tloss % opt.save_every == 0 then
		lossfile = string.format("loss_hs%d_lr%.0e_w%d_tg123.t7",opt.hiddenSize, opt.lr, opt.weight);
