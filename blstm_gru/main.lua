...
cmd:option('-decay', 1, 'if learning rate decay')
...
cmd:option('-print_every', 200, 'how many steps between printing out the loss')
cmd:option('-save_every', 5, 'how many epochs between saving the model')
--rnn layer
cmd:option('-lstm', 1, 'use Long Short Term Memory (nn.LSTM instead of nn.Recurrent)')
cmd:option('-gru', 0, 'use Gated Recurrent Units (nn.GRU instead of nn.Recurrent)')
