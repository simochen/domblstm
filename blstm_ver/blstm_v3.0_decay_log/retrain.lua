...
local function retrain(opt, trainset, cvset)
	...
	if opt.decay then
		mfile = string.format("hs%d_w%d_tg1234_ep%d.t7",opt.hiddenSize, opt.weight, opt.last_epoch)
	else
		mfile = string.format("hs%d_w%d_lr%.0e_tg1234_ep%d.t7",opt.hiddenSize, opt.weight, opt.startlr, opt.last_epoch)
	end	
		...
		if i% trainset.size == 0 then
			...
			if cverr < args.minerr then
				...
				if args.ntrial >= opt.earlystop then
					...
					if opt.decay then
						modelfile = string.format("hs%d_w%d_tg%d%d%d%d_ep%d_end.t7", opt.hiddenSize, opt.weight, opt.trainG[1], opt.trainG[2], opt.trainG[3], opt.trainG[4], epoch)
					else
						modelfile = string.format("hs%d_w%d_lr.0e_tg%d%d%d%d_ep%d_end.t7", opt.hiddenSize, opt.weight, args.lr, opt.trainG[1], opt.trainG[2], opt.trainG[3], opt.trainG[4], epoch)
					end
					print(modelfile)
