...
local function retrain(opt, trainset, cvset)
	...
	if opt.decay then
		...
		if opt.dropout <= 0 then
			mfile = string.format("hs%d_w%d_lr.0e_tg%d%d%d%d_ep%d.t7", opt.hiddenSize, opt.weight, opt.startlr, opt.trainG[1], opt.trainG[2], opt.trainG[3], opt.trainG[4], opt.epoch)
		else
			mfile = string.format("hs%d_w%d_lr.0e_dp%.2f_tg%d%d%d%d_ep%d.t7", opt.hiddenSize, opt.weight, opt.startlr, opt.dropout, opt.trainG[1], opt.trainG[2], opt.trainG[3], opt.trainG[4], opt.epoch)
		end
	end
