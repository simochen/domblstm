...
local function retrain(opt, trainset, cvset)
	...
	if opt.decay == 'log' or opt.decay == 'linear' then
		if opt.dropout <= 0 then
			mfile = string.format("%s_hs%d_w%d_cv%d_ep%d.t7", opt.decay, opt.hiddenSize, opt.weight, opt.cv, opt.last_epoch)
		else
			mfile = string.format("%s_hs%d_w%d_dp%.2f_cv%d_ep%d.t7", opt.decay, opt.hiddenSize, opt.weight, opt.dropout, opt.cv, opt.last_epoch)
		end
	else
		if opt.dropout <= 0 then
			mfile = string.format("hs%d_w%d_lr.0e_cv%d_ep%d.t7", opt.hiddenSize, opt.weight, opt.startlr, opt.cv, opt.last_epoch)
		else
			mfile = string.format("hs%d_w%d_lr.0e_dp%.2f_cv%d_ep%d.t7", opt.hiddenSize, opt.weight, opt.startlr, opt.dropout, opt.cv, opt.last_epoch)
		end
	end
