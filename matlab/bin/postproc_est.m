...
param.ep = 297;
param.sg = 1;
file = sprintf('../data/postproc/%s/log_hs10_w%d_cv%d_ep%d.mat', dir, weight, param.cv, param.ep);
load(file)
...
file = sprintf('../data/postproc/%s/log_hs10_w%d_cv%d_ep%d_sg%d.mat', dir, weight, param.cv, param.ep, param.sg);
load(file)
