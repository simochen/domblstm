...
%opt = struct('win', 5, 'cutoff', [0.55, 0.6, 0.65], 'minBlen', 5, 'minArea', [0.1, 0.1, 0.1], 'minusArea', 0.01, 'skip', 20, 'termComb', 30, 'inComb', 40, 'isShow', 0, 'pauseTime', -1);
...
for sg = 1:5
    ...
    yo = yo/5;
    seqNum = numel(vec_group{sg});
    pred(pos+1:pos+seqNum) = postproc(yo, vec_group{sg}, opt);
    for i = 1: seqNum
        def(i+pos).label = vec_group{sg}(i).yi;
        def(i+pos).range = vec_group{sg}(i).range;
        pred(i+pos).label = pred(i+pos).labelp;
    end
    pos = pos + seqNum;
end
